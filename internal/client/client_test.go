...
func TestTokenID_Success(t *testing.T) {
	srv := fakeAPI(t, map[string]http.HandlerFunc{
		"GET /api/me": func(w http.ResponseWriter, r *http.Request) {
			if !requireAuth(w, r, "s2_validtoken") {
				return
	...
	c := New(srv.URL, "s2_validtoken")
	tokenID, err := c.TokenID()
	if err != nil {
		t.Fatalf("expected success, got %v", err)
	}
	if tokenID != "01JXYZ1234ABCDEF" {
		t.Errorf("expected token_id 01JXYZ1234ABCDEF, got %s", tokenID)
	}
}

func TestTokenID_NonTokenAuth_ReturnsEmpty(t *testing.T) {
	srv := fakeAPI(t, map[string]http.HandlerFunc{
		"GET /api/me": func(w http.ResponseWriter, r *http.Request) {
			json.NewEncoder(w).Encode(map[string]any{
				"type":    "user",
	...
	if tokenID != "" {
		t.Errorf("expected empty token_id for non-token auth, got %s", tokenID)
	}
}

func TestTokenID_InvalidToken(t *testing.T) {
	srv := fakeAPI(t, map[string]http.HandlerFunc{
		"GET /api/me": func(w http.ResponseWriter, r *http.Request) {
			w.WriteHeader(401)
		},

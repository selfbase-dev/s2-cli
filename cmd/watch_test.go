...
// =============================================================================
// Layer 1d: Unit — pollFn self-change filtering (via httptest)
// =============================================================================

func TestPollFn_FiltersSelfChanges(t *testing.T) {
	ownTokenID := "tok_self_123"
	...
	tests := []struct {
		...
		{
			name: "OnlySelfChanges",
			changes: []client.ChangeEvent{
				{Seq: 1, TokenID: ownTokenID, Path: "a.txt", Action: "put"},
				{Seq: 2, TokenID: ownTokenID, Path: "b.txt", Action: "delete"},
			},
			wantChange: false,
		},
		{
			name: "OnlyOtherChanges",
			changes: []client.ChangeEvent{
				{Seq: 1, TokenID: "tok_other_456", Path: "a.txt", Action: "put"},
			},
			wantChange: true,
		},
		{
			...
			changes: []client.ChangeEvent{
				{Seq: 1, TokenID: ownTokenID, Path: "a.txt", Action: "put"},
				{Seq: 2, TokenID: "tok_other_456", Path: "b.txt", Action: "put"},
			},
			wantChange: true,
		},
		{
			name: "EmptyTokenID_NotFiltered",
			changes: []client.ChangeEvent{
				{Seq: 1, TokenID: "", Path: "a.txt", Action: "put"},
			},
			wantChange: true,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Simulate the filtering logic from pollFn
			hasChanges := false
			for _, ch := range tt.changes {
			...
			if hasChanges != tt.wantChange {
				t.Errorf("hasChanges = %v, want %v", hasChanges, tt.wantChange)
			}
		})
	}
}

func TestPollFn_NoFilterWhenTokenIDEmpty(t *testing.T) {
	// When ownTokenID is empty (e.g., TokenID() failed), all changes should trigger sync
	ownTokenID := ""
	changes := []client.ChangeEvent{
		{Seq: 1, TokenID: "tok_any", Path: "a.txt", Action: "put"},
	}

	// Simulate the fallback logic: if ownTokenID is empty, use len(changes) > 0
	var hasChanges bool
	if ownTokenID == "" {
	...
	if !hasChanges {
		t.Error("expected hasChanges=true when ownTokenID is empty")
	}
}
